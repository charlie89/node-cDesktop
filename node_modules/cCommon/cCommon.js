(function(context) {
   
   // set some globals
   __rootpath = require('path').dirname(process.mainModule.filename);

   global.trim = function (stri) {
      if (typeof stri === 'undefined') return '';
   	var str = stri.replace('/^\s\s*/', '');
   	var ws = /\s/;
   	var i = str.length;
   	while (ws.test(str.charAt(--i)));
   	return str.slice(0, i + 1);
   }

exports.secondsToDHms = function(sec) 
{
    sec = Number(sec);
    var d = Math.floor(sec / (24*3600));
    var h = Math.floor((sec - d*24*3600) / 3600);
	var m = Math.floor(sec % 3600 / 60);
	var s = Math.floor(sec % 3600 % 60);
	return ((d > 0 ? d + 'd ' : '') + (h < 10 ? '0' : '') + h + ':' + (m < 10 ? '0' : '') + m + ':' + (s < 10 ? '0' : '') + s);
};

exports.bytesToSize = function(bytes, precision)
{  
    var kilobyte = 1024;
    var megabyte = kilobyte * 1024;
    var gigabyte = megabyte * 1024;
    var terabyte = gigabyte * 1024;
   
    if ((bytes >= 0) && (bytes < kilobyte)) {
        return bytes + ' B';
 
    } else if ((bytes >= kilobyte) && (bytes < megabyte)) {
        return (bytes / kilobyte).toFixed(precision) + ' KB';
 
    } else if ((bytes >= megabyte) && (bytes < gigabyte)) {
        return (bytes / megabyte).toFixed(precision) + ' MB';
 
    } else if ((bytes >= gigabyte) && (bytes < terabyte)) {
        return (bytes / gigabyte).toFixed(precision) + ' GB';
 
    } else if (bytes >= terabyte) {
        return (bytes / terabyte).toFixed(precision) + ' TB';
 
    } else {
        return bytes + ' B';
    }
};

})(exports);